...
class PhraseModel(nn.Module):
	def __init__(self, emb_size, hid_size):
		...
		self.encoder = nn.LSTM(input_size=emb_size, hidden_size=hid_size,num_layers=1, batch_size=True)
		self.decoder = nn.LSTM(input_size=emb_size,hidden_size=hid_size,num_layers=1,batch_size=True)
		...
		def pack_batch_no_out(batch, embeddings, device="gpu"):
		...
		def pack_input(input_data, embeddings, device="gpu"):
		...
		def pack_batch(batch, embeddings, device="gpu"):
